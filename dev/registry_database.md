...
- `registry_type`: Specifies the type of the model in the row: `project`, `dataset`, `dimension`,
`dimension_mapping`.
...
**Note**: `model_id` + `version` + `registry_type` is a secondary way to uniquely identify a row.
...
```
sqlite> SELECT DISTINCT model_id FROM models WHERE registry_type = 'project';
```
```
+-------------------+
|     model_id      |
...
```
sqlite> SELECT id, model_id, version FROM models WHERE registry_type = 'project';
```
```
+-----+-------------------+---------+
| id  |     model_id      | version |
...
```
sqlite> SELECT DISTINCT model_id FROM models WHERE registry_type = 'dataset';
```
```
+--------------------------------------------------------------+
|                           model_id                           |

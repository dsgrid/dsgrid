...
| `expected_associations` | list[`str`] | `[]` | List of paths to expected associations files (e.g., expected_associations.parquet) or directories of files containing expected combinations by dimension type (e.g., geography__subsector.csv, subsector__metric.csv). When provided, only these dimension combinations are required to be present in the data, rather than the full cross-join of all dimension records. |

...
class DatasetConfigModel(DSGBaseDatabaseModel):
    ...
    def check_time_zone(self) -> "DatasetConfigModel":
        ...
        if isinstance(time_dim, DateTimeDimensionModel):
            needs_registration_localization = (
                time_dim.column_format.dtype == "timestamp_ntz"
                or (
                    time_dim.column_format.dtype == "time_format_in_parts"
                    and time_dim.column_format.offset_column is None
                )
            )

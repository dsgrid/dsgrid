...
def try_load_dimension_associations(project_id, dataset_id) -> pyspark.sql.DataFrame | None:
    ...
    """
    return try_load_stored_table(_make_dimension_associations_table_name(project_id, dataset_id))


def save_dimension_associations(table, project_id, dataset_id):
    ...
    """
    table_name = _make_dimension_associations_table_name(project_id, dataset_id)
    save_table(table, table_name)
    logger.info(
        "Saved dimension associations for project_id=%s dataset_id=%s",
        project_id,
        dataset_id,
    )
...
def _make_dimension_associations_table_name(project_id, dataset_id):
    return "__".join((project_id, dataset_id, _ASSOCIATIONS_DATA_TABLE))

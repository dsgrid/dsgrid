...
from dsgrid.dimension.time import TimeIntervalType, TimeZoneFormat
from dsgrid.exceptions import DSGInvalidParameter
from dsgrid.time.types import IndexTimestampType
from .dimensions import AlignedTimeSingleTimeZone, IndexTimeDimensionModel
...
class IndexTimeDimensionConfig(TimeDimensionBaseConfig):
    ...
    def to_chronify(
        ...
        match self.model.time_zone_format.format_type:
            ...
            case _:
                msg = (
                    "Unsupported time zone format for chronify: "
                    f"{self.model.time_zone_format.format_type}"
                )
    ...
    def get_time_zone(self) -> str | None:
        # Deliberately format-based, not len(time_zones)==1: a single-entry
        # LocalTimeMultipleTimeZones must not flip get_start_times() to
        # tz-aware, because IndexTimeRangeWithTZColumn expects a naive start.
        if isinstance(self.model.time_zone_format, AlignedTimeSingleTimeZone):

...
from dsgrid.ibis_api import write_dataframe
...
class DatasetMappingManager:
    ...
    def try_read_checkpointed_table(self) -> ir.Table | None:
        """Read the checkpointed table for the dataset as Ibis Table, if it exists."""
        if self._checkpoint is None:
            return None
        return get_ibis_connection().read_parquet(self._checkpoint.persisted_table_filename)
    ...
    def persist_table(self, data: ir.Table, op: MapOperation) -> ir.Table:
        """Persist the intermediate table to the filesystem and return the persisted Table."""
        persisted_file = self._scratch_dir_context.get_temp_filename(
            suffix=".parquet", add_tracked_path=False
        )

        write_dataframe(data, persisted_file)
        self.save_checkpoint(persisted_file, op)
        logger.info("Persisted mapping operation name=%s to %s", op.name, persisted_file)
        return get_ibis_connection().read_parquet(persisted_file)

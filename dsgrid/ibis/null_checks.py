...
def check_for_nulls(df, exclude_columns=None) -> None:
    ...
    for every checked column. The pre-Phase-11 implementation executed an
    OR-of-IS-NULL filter to detect failure and then N additional queries
    (one per column) to identify the culprits; the single-query rewrite
    replaces those N+1 round-trips with one.

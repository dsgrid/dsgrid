...
class RegistryDatabase:
    ...
    def copy(
        ...
        with sqlite3.connect(src_conn.url.replace("sqlite:///", "")) as src:
            with dst.engine.connect() as dst_conn_:
                ...
                orig_data_path = dst_conn_.execute(stmt).fetchone()[0]
    ...
    def _get_model_id(self, registry_type: RegistryType, model: dict[str, Any]) -> str:
        return model[self._get_model_id_field(registry_type)]

    @staticmethod
    def _get_model_id_field(registry_type: RegistryType) -> str:
        return MODEL_ID_TABLE_MAPPING[registry_type]
    ...
    def insert_model(
        ...
        model_id = self._get_model_id(registry_type, model)
        if self.has(conn, registry_type, model_id):
            msg = f"{registry_type=} {model_id}"
        ...
        self._set_current_version(conn, registry_type, model_id, db_id)
        logger.debug("Inserted registry_type=%s model_id=%s id=%s", registry_type, model_id, db_id)
        return new_model
    ...
    def update_model(
        ...
        db_id = res.lastrowid
        new_model = self._add_database_id(conn, table, db_id)
        self._update_current_version(conn, registry_type, model_id, db_id)
        logger.error("Updated registry_type=%s model_id=%s id=%s", registry_type, model_id, db_id)
        return new_model
    ...
    def _set_current_version(
        self, conn: Connection, registry_type: RegistryType, model_id: str, db_id: int
    ) -> None:
        ...
        logger.debug("Set the current version of %s %s to %s", registry_type, model_id, db_id)
    ...
    def _update_current_version(
        ...
        stmt = (
            update(table)
            .where(table.c.registry_type == registry_type)
    ...
    def get_containing_models(
        self, conn: Connection, db_id: int, registry_type: Optional[RegistryType] = None
    ...
    def get_containing_models_by_version(
        ...
        return self.get_containing_models(conn, db_id, registry_type=registry_type)
    ...
    def delete_models(self, conn: Connection, registry_type: RegistryType, model_id: str) -> None:
        """Delete all documents with the model_id."""
        for table in (RegistryTables.MODELS, RegistryTables.CURRENT_VERSIONS):
            table = self._get_table(table)
            stmt = (
                delete(table)
                .where(table.c.registry_type == registry_type.value)
                .where(table.c.model_id == model_id)
            )
    ...
    def get_latest(self, conn: Connection, registry_type: RegistryType, model_id: str):
        return self._get_latest_column(conn, registry_type, model_id, "model")

    def get_latest_version(
        self, conn: Connection, registry_type: RegistryType, model_id: str
    ) -> str:
        return self._get_latest_column(conn, registry_type, model_id, "version")

    def _get_latest_column(
        self, conn: Connection, registry_type: RegistryType, model_id: str, column: str
    ) -> Any:
        ...
        stmt = (
            ...
            .where(table2.c.registry_type == registry_type)
            .where(table2.c.model_id == model_id)
        )
        res = conn.execute(stmt).fetchall()
        if not res:
            msg = f"{registry_type=} {model_id=} is not registered"
    ...
    def list_model_ids(self, conn: Connection, registry_type: RegistryType) -> list[str]:
        table = self.get_table(RegistryTables.MODELS)
        stmt = select(table.c.model_id).where(table.c.registry_type == registry_type).distinct()
        return [x[0] for x in conn.execute(stmt).fetchall()]

    def iter_models(
        self, conn: Connection, registry_type: RegistryType, all_versions: bool = False
    ) -> Generator[dict[str, Any], None, None]:
        table = self.get_table(RegistryTables.MODELS)
        if all_versions:
            stmt = select(table.c.model).where(table.c.registry_type == registry_type)
        else:
            table2 = self._get_table(RegistryTables.CURRENT_VERSIONS)
            stmt = (
                select(
    ...
    def _get_by_version(
        self, conn: Connection, registry_type: RegistryType, model_id: str, version: str
    ):
        table = self.get_table(RegistryTables.MODELS)
        stmt = (
        ...
        if not res:
            ...
            raise DSGValueNotRegistered(msg)
        if len(res) > 1:
            msg = f"Bug: found more than one row. {registry_type=} {model_id=} {version=}"
            raise Exception(msg)
        return res[0][0]
    ...
    def get_initial_registration(
        ...
        stmt = (
            ...
            .join_from(table1, table2, table1.c.registration_id == table2.c.id)
            .where(table1.c.registry_type == registry_type.value)
...
def create_tables(engine: Engine, metadata: MetaData) -> None:
    ...
    models_table = Table(
        ...
        Column("registry_type", String(), nullable=False),
        ...
        UniqueConstraint("registry_type", "model_id", "version"),
    )
    Table(
        ...
        UniqueConstraint("registry_type", "model_id"),
    )
...
if __name__ == "__main__":
    registry = RegistryDatabase.create(
        DatabaseConnection(url="sqlite:///tmpdsgrid.db"), Path("tmp_dsgrid_data"), overwrite=True
    )

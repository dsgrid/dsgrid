...
class RegistryModel(DSGBaseDatabaseModel):
...
class RegistryInterfaceBase(abc.ABC):
    ...
    @staticmethod
    @abc.abstractmethod
    def _registry_type() -> RegistryType:
    ...
    def delete_all(self, conn: Optional[Connection], model_id: str) -> None:
        ...
        if conn is None:
            with commit_manager(self._db.engine) as conn:
                return self._db.delete_models(conn, self._registry_type(), model_id)
        return self._db.delete_models(conn, self._registry_type(), model_id)
    ...
    def get_latest_version(self, conn: Optional[Connection], model_id: str) -> str:
        ...
        if conn is None:
            with self._db.engine.connect() as conn:
                return self._db.get_latest_version(conn, self._registry_type(), model_id)
        return self._db.get_latest_version(conn, self._registry_type(), model_id)
    ...
    def has(
        ...
        if conn is None:
            with self._db.engine.connect() as conn:
                return self._db.has(conn, self._registry_type(), model_id, version=version)
        return self._db.has(conn, self._registry_type(), model_id, version=version)
    ...
    def _insert(
        ...
        new_model = self._make_dsgrid_model(
            self._db.insert_model(
                conn, self._registry_type(), model.model_dump(mode="json"), registration
            )
        )
    ...
    def list_model_ids(self, conn: Optional[Connection] = None) -> list[str]:
        ...
        if conn is None:
            with self._db.engine.connect() as conn:
                return self._db.list_model_ids(conn, self._registry_type())
        return self._db.list_model_ids(conn, self._registry_type())
    ...
    def _iter_models(
        ...
        for data in self._db.iter_models(conn, self._registry_type(), all_versions=all_versions):
    ...
    def get_containing_models_by_version(
        ...
        if conn is None:
            with self._db.engine.connect() as conn:
                return self._get_containing_models_by_version(
                    conn, model_id, version, registry_type
                )
        return self._get_containing_models_by_version(conn, model_id, version, registry_type)
    ...
    def _get_containing_models_by_version(
        ...
        for registry_type, data in self._db.get_containing_models_by_version(
            conn, model_id, version, registry_type=registry_type
        ):
            model = _INTERFACE_BY_TYPE[registry_type]._make_dsgrid_model(data)
            results[registry_type].append(model)
        return results
    ...
    def get_containing_models(
        ...
        if conn is None:
            with self._db.engine.connect() as conn:
                return self._get_containing_models(conn, model, registry_type=registry_type)
        return self._get_containing_models(conn, model, registry_type=registry_type)
    ...
    def _get_containing_models(
        ...
        results = {x: [] for x in RegistryType}
        for registry_type, data in self._db.get_containing_models(
            conn, model.id, registry_type=registry_type
        ):
            model = _INTERFACE_BY_TYPE[registry_type]._make_dsgrid_model(data)
            results[registry_type].append(model)
        return results
    ...
    def _get_by_version(self, conn: Optional[Connection], model_id, version) -> dict[str, Any]:
        if conn is None:
            with self._db.engine.connect() as conn:
                return self._db._get_by_version(conn, self._registry_type(), model_id, version)
        return self._db._get_by_version(conn, self._registry_type(), model_id, version)

    def _get_latest(self, conn: Optional[Connection], model_id: str) -> dict[str, Any]:
        if conn is None:
            with self._db.engine.connect() as conn:
                return self._db.get_latest(conn, self._registry_type(), model_id)
        return self._db.get_latest(conn, self._registry_type(), model_id)

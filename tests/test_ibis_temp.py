...
def test_track_temp_file_removes_directory(monkeypatch, tmp_path):
    # A partitioned parquet output is a directory of part files; cleanup must
    # remove the whole directory, not raise IsADirectoryError and leak it.
    monkeypatch.setattr(dsgrid.runtime_config, "backend_engine", BackendEngine.DUCKDB)
    tracked = tmp_path / "leaked_parquet_dir"
    tracked.mkdir()
    (tracked / "part-00000.parquet").write_bytes(b"")
    (tracked / "part-00001.parquet").write_bytes(b"")
    track_temp_file(tracked)
    drop_temp_tables_and_views()

...
tz-naive `start`, while the DataFrame's time column is tz-aware. Without
...
`dsgrid.utils.dataset`, so it covers all three chronify map dispatchers (DuckDB,
Spark+path, Spark+Hive). The test body is backend-agnostic: it dispatches via
...
import dsgrid
from dsgrid.spark.functions import init_spark
...
from dsgrid.utils.spark import persist_table, read_dataframe
...
def _map_time_dim(df, from_time_dim, to_time_dim, scratch_dir_context):
    ...
    if dsgrid.runtime_config.use_hive_metastore:
        from dsgrid.spark.functions import make_temp_view_name, save_to_warehouse

        table_name = make_temp_view_name()
        return map_time_dimension_with_chronify_spark_hive(
    ...
    return map_time_dimension_with_chronify_spark_path(
...
def test_ntz_config_with_tz_aware_dataframe(scratch_dir_context):
    """from_time_dim says NTZ, but the DataFrame is post-localization (tz-aware).

...
entry point, :func:`dsgrid.units.convert.convert_units_unpivoted`. The
prior SQL-string helpers in dsgrid.units.energy and dsgrid.units.power
had no production callers and were deleted in Phase 7; this file now
exercises the Ibis-based ``convert_units_unpivoted`` directly.
"""
...
def test_convert_units_unpivoted_with_mapping(energy_from_records):
    ...
    to_records = create_dataframe_from_dicts(
        [{"id": f"{i}_out", "unit": KWH} for i in ["fans", "cooling", "dryer", "ev_l1l2", "ng_heating", "p_heating"]]
    )
    result = convert_units_unpivoted(
        df,
        metric_column="metric",
